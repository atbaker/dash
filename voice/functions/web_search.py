...
async def web_search(params: FunctionCallParams):
    """
    Perform web search by calling DigitalOcean Function via HTTP.
    ...
    try:
        ...
        function_url = f"{base_url}/web_search"
        ...
        await params.result_callback({"error": "Web search request timed out"})
    except requests.exceptions.RequestException as e:
        await params.result_callback({"error": "Network error", "details": str(e)})
    except Exception as e:
        await params.result_callback({"error": f"Web search error: {str(e)}"})